...
export const Employee = () => {
    ...
    useEffect(() => {
        ...
        const getEmployee = async () => {
            const res = await employeesAPI.getEmployeeClustersById(String(id))
    ...
    const saveChanges = () => {
        if (!reason) {
            ...
            employeesAPI.changeProperties(String(id), reason, changedProperties)
                ...
                .catch(() => message.error('Во время изменения данных произошла ошибка :('))
    ...
    const changeClusterField = (
        ...
        propId: string,
        propType: string,
        newValue: string | number | undefined | ClusterPropertyValue['enumValue']
    ) => {
        ...
        setChangedProperties(prev => {
            ...
            if (prop) {
                prop.newValue = typeof newValue !== 'string' && typeof newValue !== 'number' ? newValue?.id as string : newValue
                return [...prev]
            }
            prop = {
                idProperty: propId,
                newValue: typeof newValue !== 'string' && typeof newValue !== 'number' ? newValue?.id as string : newValue
            }
            return [...prev, prop]
        })
    }
