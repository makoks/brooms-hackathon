...
import { Space, Input, Button, message, Row, Col, Switch, Typography } from "antd";
...
export const EnumList: React.FC<EnumListProps> = ({ propertyId }) => {
	...
	const changeEnumItem = async (id: string, name: string, point: number) => {
						...
						: <Row gutter={24} justify='space-between' align='middle' style={{ padding: '0 12px' }}>
							...
							<Col>
								<Space size={8}>
									...
									<Switch title='Задать веса' checked={withWeights} onChange={setWithWeights} />
								</Space>
							</Col>
						</Row>
					}
